...
1. **Research — set up the problem.** “This is the PreStocks company view. The quote, reference mark, company headlines, and update time are together.”
2. **Show the Catalyst Monitor.** Choose a company-linked headline and point out its publisher, date, nearby pool closes, liquidity, and volume. Then show its market flags. If the company is saved, point to the sidebar check: “It shows how the quote moved since my last check and flags weak pool data. It refreshes every ten minutes while the app is open; it is not a trading alert.” The nearby-close comparison shows timing, not that the headline caused a move.
...
**Why use multiple AI roles?**

One summary can leave out the strongest counterargument. Bull, Bear, and Neutral receive the same headlines and market snapshot. A Council chair compares the three cases and reports a research read, not a vote on what will happen. Model names are disclosed so users can see which system answered each role.

**Does the AI read the articles?**

No. It receives public headline metadata and the displayed market snapshot, not full article text. Its claims must point to the evidence packet when they rely on a headline.

**Why Solana and PreStocks?**

Vestra is built around assets in the PreStocks catalogue and follows the selected mint to a Solana pool. This keeps the company quote and the token’s on-chain activity visible in the same research flow. The PreStocks bounty is the closest track fit; Vestra only uses PreStocks catalogue assets.
...
The app shows a retry state instead of inventing an answer. Retry once if time allows; otherwise continue through the live headlines and Market replay and describe the debate flow accurately.

## Track and entry

- **Primary fit:** PreStocks — Best Use of PreStocks.
- **Also enter:** Main Track.
- The live form still needs an accessible judge link, team details, and the submitting account/wallet. Check the event page for the current deadline before submitting. See [SUBMISSION.md](./SUBMISSION.md) for the copy-ready entry and the remaining access requirements.
