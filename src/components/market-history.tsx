...
type Backtest = { strategyReturn: number; buyHoldReturn: number; maxDrawdown: number; closedTrades: number; openPosition: boolean; winRate: number; observations: number; windowStart: number; windowEnd: number };

function simulateSma(candles: Candle[], costBps: number): Backtest | null {
  const fast = 5;
  const slow = 20;
  ...
  for (let i = slow - 1; i < candles.length - 1; i += 1) {
    const fastAverage = candles.slice(i - fast + 1, i + 1).reduce((sum, candle) => sum + candle.close, 0) / fast;
    const slowAverage = candles.slice(i - slow + 1, i + 1).reduce((sum, candle) => sum + candle.close, 0) / slow;
    const nextPosition = fastAverage > slowAverage;
    const next = candles[i + 1];
    const buyHoldChange = next.close / candles[i].close;
    buyHoldEquity *= buyHoldChange;
    let intervalReturn = 1;
    if (position) {
      const overnight = next.open / candles[i].close;
      intervalReturn *= overnight;
      tradeReturn *= overnight;
    }
    if (position !== nextPosition) {
      intervalReturn *= 1 - cost;
      if (position) {
        trades += 1;
        if (tradeReturn * (1 - cost) > 1) wins += 1;
        tradeReturn = 1;
      } else {
        tradeReturn *= 1 - cost;
      }
    ...
    if (nextPosition) {
      const intraday = next.close / next.open;
      intervalReturn *= intraday;
      if (position || nextPosition) tradeReturn *= intraday;
    }
    strategyEquity *= intervalReturn;
    position = nextPosition;
    peak = Math.max(peak, strategyEquity);
    maxDrawdown = Math.max(maxDrawdown, (peak - strategyEquity) / peak);
  }
  // Mark any final open position to the last close; no exit order is assumed.
  return { strategyReturn: strategyEquity - 1, buyHoldReturn: buyHoldEquity - 1, maxDrawdown, closedTrades: trades, openPosition: position, winRate: trades ? wins / trades : 0, observations: candles.length - slow, windowStart: candles[slow - 1].time, windowEnd: candles[candles.length - 1].time };
}
...
export function MarketHistory({ mint, ticker, tokenPrice, articles = [] }: { mint: string; ticker: string; tokenPrice: number | null; articles?: Article[] }) {
  ...
  const poolComparison = poolQuoteGap === null
    ? "Could not compare the pool price with PreStocks"
    : `The selected USDC pool's latest USD close is ${Math.abs(poolQuoteGap).toFixed(1)}% ${poolQuoteGap >= 0 ? "above" : "below"} the PreStocks catalogue quote. This is not a trade signal`;
        ...
        <div className="details-body"><div className="backtest-panel">
          ...
          {backtest ? <>
            <div className="backtest-stats">
              <div><span className="eyebrow">STRATEGY RESULT</span><strong className={backtest.strategyReturn >= 0 ? "positive" : "negative"}>{backtest.strategyReturn >= 0 ? "+" : ""}{(backtest.strategyReturn * 100).toFixed(2)}%</strong></div>
              <div><span className="eyebrow">RETURN IF HELD</span><strong className={backtest.buyHoldReturn >= 0 ? "positive" : "negative"}>{backtest.buyHoldReturn >= 0 ? "+" : ""}{(backtest.buyHoldReturn * 100).toFixed(2)}%</strong></div>
              <div><span className="eyebrow">LARGEST DROP</span><strong className="negative">−{(backtest.maxDrawdown * 100).toFixed(2)}%</strong></div>
              <div><span className="eyebrow">COMPLETED TRADES</span><strong>{backtest.closedTrades}</strong></div>
              <div><span className="eyebrow">AT THE END</span><strong>{backtest.openPosition ? "Holding" : "Cash"}</strong></div>
              <div><span className="eyebrow">WINNING TRADES</span><strong>{backtest.closedTrades ? `${(backtest.winRate * 100).toFixed(0)}%` : "—"}</strong></div>
              <div><span className="eyebrow">DAYS MEASURED</span><strong>{backtest.observations}</strong></div>
            </div>
            <p className="backtest-disclosure">Sample only: uses Solana pool prices, not PreStocks returns or investment advice. Period: {candleDate(backtest.windowStart)} – {candleDate(backtest.windowEnd)} UTC ({backtest.observations} daily prices). It compares 5-day and 20-day averages; gaps in trading can make these periods longer. Trades use the next available day’s opening price. The selected cost is charged on each buy and sell. An open position is valued at the final close, as if still held. Taxes, funding costs, and the effect of large trades are excluded. {thinActivity ? "Very thin trading makes results especially uncertain." : "Limited trading and price history make results uncertain."}</p>
          </> : <p className="catalyst-empty">At least 21 daily prices are needed.</p>}
        </div></div>
      </details>
